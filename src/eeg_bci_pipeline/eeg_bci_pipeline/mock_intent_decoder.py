...
from eeg_bci_pipeline.node_params import float_param, int_param, str_list_param, str_param
...
class MockIntentDecoder(Node):
    ...
    def __init__(self) -> None:
        ...
        self._class_labels = tuple(str_list_param(self, "class_labels"))
    ...
    def _on_eeg_frame(self, frame: EegFrame) -> None:
        ...
        if self._expected_channel_labels is None:
            ...
            self._expected_channel_labels = self._normalized_labels(channel_labels)
            self.get_logger().info(
        ...
        prediction = decode_mock_intent(samples, self._class_labels)
        intent = Intent()
    ...
    def _read_optional_labels(self, parameter_name: str) -> tuple[str, ...] | None:
        ...
        labels = self._normalized_labels(value)
